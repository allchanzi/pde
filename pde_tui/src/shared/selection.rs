...
pub fn first(state: &mut ListState, len: usize) {
    state.select((len > 0).then_some(0));
}

pub fn last(state: &mut ListState, len: usize) {
    state.select(len.checked_sub(1));
}

pub fn page_down(state: &mut ListState, len: usize, page_size: usize) {
    state.select(offset_index(state.selected(), len, page_size));
}

pub fn page_up(state: &mut ListState, len: usize, page_size: usize) {
    state.select((len > 0).then(|| state.selected().unwrap_or(0).saturating_sub(page_size)));
}

fn offset_index(selected: Option<usize>, len: usize, offset: usize) -> Option<usize> {
    (len > 0).then(|| (selected.unwrap_or(0) + offset).min(len - 1))
}
